...
function normalizeIDBPath(path) {
    if (!path.startsWith('/')) {
        path = '/' + path;
    }
 ...
 */
export async function readBinaryIDB(path) {
    try {
 ...
 */
export async function writeBinaryIDB(path, content) {
    const db = await getIDB();
    const normalized = normalizeIDBPath(path);
    const now = Date.now();
    return new Promise((resolve, reject) => {
 ...
 */
export async function existsIDB(path) {
    try {
        const db = await getIDB();
        const normalized = normalizeIDBPath(path);
        return new Promise((resolve, reject) => {
 ...
 */
export async function readBinaryBatchIDB(paths) {
                ...
                request.onsuccess = () => {
                    ...
                    if (file) {
                        if (file.isBinary) {
                            ...
                            const encoder = new TextEncoder();
                            results.set(path, encoder.encode(file.content));
                        }
                    }
                    pending--;
                    if (pending === 0)
...
function getOPFSPathParts(path) {
    const normalized = normalizeOPFSPath(path);
    if (!normalized)
        return [];
    return normalized.split('/');
}
async function getOPFSParentAndName(path) {
 ...
 */
export async function readBinaryOPFS(path) {
    try {
 ...
 */
export async function writeBinaryOPFS(path, content) {
    const parts = getOPFSPathParts(path);
    if (parts.length === 0) {
 ...
 */
export async function existsOPFS(path) {
    try {
        const { parent, name } = await getOPFSParentAndName(path);
        try {
 ...
 */
export async function readBinaryBatchOPFS(paths) {
    const results = new Map();
 ...
 */
export async function writeBinary(path, content) {
    if (isOPFSAvailable()) {
        try {
